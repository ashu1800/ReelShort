...
[2026-07-17] backend/admin/order - VIP 收款扫描改为按订单地址与合约快照分组，新增 V23 持久化 fingerprint cursor 以跨轮续扫分页硬上限；精确候选 receipt 未成熟或确认数不足时停留当前页重试，超出付款有效期与确认宽限后才继续；手工确认按 txHash、地址、合约和应付金额精确选择 Transfer event；补齐并发尾差、权益续期和真实后台 RBAC 测试；已启用 TOTP 的管理员登录必须同时提交 6 位动态码。
