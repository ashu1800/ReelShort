...
- `android-app`：Android 原生客户端，使用 Kotlin、Jetpack Compose、Material3 和 Material Icons 建立登录、首页、搜索、详情、播放、账户页面骨架；Compose UI 已接入 `AppStateController`/`AppUiState`，并具备深色影院感视觉基础层；纯 Kotlin `app-core` 模块承载 Spring Boot API 配置、OkHttp 客户端接口实现、会话存储边界、Repository、数据源边界、共享模型、播放状态、UI action facade 和可测试的 UI 状态控制层，后续接入原生媒体播放和平台持久化能力。
...
[2026-06-28] android-app/images - 设计 Android 内容封面图基础层，规划 Coil Compose、网络权限、封面展示和 fallback 海报。
