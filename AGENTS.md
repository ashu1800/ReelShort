...
- `admin-web`：后台管理网站，使用 Vue、Element Plus、Pinia、Vue Router 和 Axios，包含管理员登录、会话、基础导航、用户运营操作、BLACKLISTED 状态管理、提现申请拒绝与单笔/批量自动打款、打款 attempt 状态和链上确认进度、用户积分交易记录、订单管理、内容缓存单语言刷新、双语货架刷新、货架健康状态和刷新任务状态、系统配置、运行诊断、内容源结构化诊断事件、异常告警、系统日志和审计日志视图；提现预览只提交提现 ID，私钥仅在最终执行请求中随 6 位 TOTP 提交，并在请求结束、弹层关闭和页面离开时清除。
...
[2026-07-17] backend/admin-web/withdrawal - 后台提现管理对接持久化 payout attempt：预览接口移除私钥和派生余额查询，单笔/批量执行按网络逐次提交私钥与 6 位 TOTP，仅 PREPARED、BROADCASTED、CONFIRMED 计为已提交，列表展示广播、确认、失败和人工核对状态，成功与失败资金操作写独立审计，前端覆盖所有退出路径的凭据清理。
