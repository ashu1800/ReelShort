...
- `android-app/release`：通过 `shortlink.hjj888.cc` 自托管版本清单检测稳定版，在 App 私有缓存流式下载限速 APK 并显示进度，经 SHA-256、包名、版本和签名校验后调用 Android 系统安装器；GitHub Actions 仅使用仓库 Secrets 中的正式签名材料构建 APK，并通过专用低权限 SSH 用户原子上传到自有服务器。
