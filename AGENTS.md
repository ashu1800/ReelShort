...
- `backend/points`：积分账户、观看奖励、后台积分调整、充值入账和用户间积分交易模块；观看奖励和运营模拟观看共用 `points.daily-earned.maximum` 每账号每日自动获取上限，按服务器自然日重置且只限制 `WATCH_REWARD` 自动奖励；积分交易按完整手机号账号归一化接收方，不扣手续费，并基于可用积分执行同事务扣减和入账；积分余额增加和入账路径带整型溢出保护，接收方溢出时转账整体回滚。
