...
[2026-07-17] backend/admin-web/withdrawal - 后台提现管理对接持久化 payout attempt：预览接口移除私钥和派生余额查询，单笔/批量执行按网络逐次提交私钥与 6 位 TOTP，列表展示广播、确认、失败和人工核对状态，成功与失败资金操作写独立审计，前端覆盖所有退出路径的凭据清理。
