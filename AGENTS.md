...
- `android-app`：Android 原生客户端，使用 Kotlin、Jetpack Compose、Material3 和 Material Icons 建立游客首页、搜索、详情、播放、账户和登录/注册底部面板；Compose UI 已接入 `AppStateController`/`AppUiState`，默认启动进入首页，未登录可浏览内容，播放和账户数据等受保护动作触发底部认证面板；首页、搜索和收藏入口点击短剧后加载分集并直接进入播放，未登录时停留原列表弹出认证面板，登录成功后自动继续播放；已登录用户点击短剧时优先按后端观看历史恢复最近观看集数，已看完集数默认续到下一集；首页、搜索和详情海报支持 Coil 加载内容封面，并保留渐变 fallback；详情页展示短剧简介，分集行展示上游标题和分集简介并在缺失时回退短剧简介；播放页接入 Media3/ExoPlayer 播放 Spring Boot 返回的合法 HTTP/HTTPS 媒体 URL，播放器采用 9:16 竖屏优先布局、自动播放并从历史快照位置恢复，视频准备或缓冲期间显示播放器内加载状态，底部提供选集入口和分集弹层；播放页在 25/50/75/100 观看奖励阶段自动静默上报，并在播放器右上角展示圆形奖励进度状态；账户页采用会员中心式“我的”布局，包含身份摘要卡、2 列快捷业务入口、继续观看卡和低优先级语言/诊断/退出分组；纯 Kotlin `app-core` 模块承载 Spring Boot API 配置、OkHttp 客户端接口实现、会话存储边界、记住密码凭据存储边界、Repository、数据源边界、共享模型、播放状态和可测试的 UI 状态控制层；生产默认 API 地址为 `https://reelshort.hjj888.cc/api/app`；Android 层使用 AndroidX Security Crypto 加密保存登录会话和记住密码凭据，并关闭 App 自动备份和全局明文流量。
- `admin-web`：后台管理网站，使用 Vue、Element Plus、Pinia、Vue Router 和 Axios，包含管理员登录、会话、基础导航、用户运营操作、订单管理、内容缓存刷新和刷新任务状态、系统配置、运行诊断、异常告警、系统日志和审计日志视图。
...
- `backend/system`：后端通用基础设施，包含统一响应、请求 ID、错误处理、用户级并发协调、后台系统配置、后台运行诊断、内容源诊断摘要、异常告警、受限系统日志查看，以及可配置内存/Redis 存储的后端限流。
