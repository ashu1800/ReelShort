...
- `android-app`：Android 原生客户端，使用 Kotlin 和 Jetpack Compose 建立登录、首页、搜索、详情、播放、观看记录、积分和订单页面骨架；纯 Kotlin `app-core` 模块承载 Spring Boot API 配置、OkHttp 客户端接口实现、Repository、数据源边界、共享模型和可测试的 UI 状态控制层，后续只访问 Spring Boot API 并接入原生媒体播放能力。
