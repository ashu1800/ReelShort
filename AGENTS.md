...
- `backend/withdrawal`：积分提现申请、后台逐次提交私钥的内存签名、持久化签名交易广播与链上确认模块；默认 50 积分兑换 1 CNY，再按后台维护的 CNY/USD 汇率折算 USDT，最低提现默认 10 USD，每笔申请保存汇率快照；TRON/ERC20/BEP20 预期热钱包公开地址为可选白名单，未配置时使用私钥派生地址，配置后必须匹配，签名阶段始终复核派生地址与持久化 intent；TRON 节点返回的 `raw_data_hex` 通过 Trident protobuf 按本地打款 intent 严格校验后才签名，首次意图字段不匹配时只重建一次，第二次返回字段级安全原因码，且日志不记录私钥、签名或完整原始交易；EVM nonce 分配与 `SIGNING` intent 同事务，所有打款状态 mutation 固定按 withdrawal -> attempt 加锁；TRON 过期且单节点未找到时进入人工核对，禁止自动重签；提交提现时冻结可用积分，达到链上确认数后正式扣减积分，拒绝时释放冻结积分。
