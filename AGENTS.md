...
- `backend/withdrawal`：ERC20 积分提现申请与管理员外部钱包确认模块；默认 50 积分兑换 1 CNY，再按后台维护的 CNY/USD 汇率折算 USDT，最低提现默认 10 USD，每笔申请保存汇率快照；提交提现时冻结可用积分，管理员以 TOTP 确认已通过外部钱包打款后正式扣减冻结积分，拒绝时释放冻结积分；系统不接收私钥或交易哈希，不签名、不广播、不做链上确认，旧自动打款 HTTP 接口固定返回 410；统计按 `reviewedAt` 在上海时区汇总已确认 ERC20 提现。
