...
- `admin-web`：后台管理网站，使用 Vue、Element Plus、Pinia、Vue Router 和 Axios，包含管理员登录、会话、基础导航、用户运营操作、BLACKLISTED 状态管理、ERC20 提现申请拒绝与管理员外部钱包打款后的单笔确认、按今天/昨天/本周/本月/上月筛选的 ERC20 打款笔数和金额统计、用户积分交易记录、带失败重试幂等键的积分调整、订单管理、内容缓存单语言刷新、双语货架刷新、货架健康状态和刷新任务状态、系统配置、运行诊断、内容源结构化诊断事件、异常告警、系统日志和审计日志视图；管理员登录支持可选 6 位 TOTP，已启用 2FA 的账号由后端强制验证；提现确认只要求已登录且具备 `WITHDRAWAL_WRITE`，不接收私钥、交易哈希或二次验证码，不签名、不广播、不查询链上交易。
...
- `backend/withdrawal`：ERC20 积分提现申请与管理员外部钱包确认模块；默认 50 积分兑换 1 CNY，再按后台维护的 CNY/USD 汇率折算 USDT，最低提现默认 10 USD，每笔申请保存汇率快照；提交提现时冻结可用积分，已登录且具备 `WITHDRAWAL_WRITE` 的管理员确认已通过外部钱包打款后正式扣减冻结积分，拒绝时释放冻结积分；系统不接收私钥、交易哈希或二次验证码，不签名、不广播、不做链上确认，旧自动打款 HTTP 接口固定返回 410；统计按 `reviewedAt` 在上海时区汇总已确认 ERC20 提现。
