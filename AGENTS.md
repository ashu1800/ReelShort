...
[2026-07-17] backend/admin/order - VIP 收款扫描改为按订单地址与合约快照分组，新增 V23 持久化 fingerprint cursor 以跨轮续扫分页硬上限；手工确认改为按 txHash 直查 Transfer events，补齐并发尾差、权益续期和真实后台 RBAC 测试；已启用 TOTP 的管理员登录必须同时提交 6 位动态码。
