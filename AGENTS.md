...
- `android-app`：Android 原生客户端，使用 Kotlin 和 Jetpack Compose 建立登录、首页、搜索、详情、播放、账户页面骨架；Compose UI 已接入 `AppStateController`/`AppUiState`，纯 Kotlin `app-core` 模块承载 Spring Boot API 配置、OkHttp 客户端接口实现、会话存储边界、Repository、数据源边界、共享模型、UI action facade 和可测试的 UI 状态控制层，后续接入原生媒体播放和平台持久化能力。
- `admin-web`：后台管理网站，使用 Vue、Element Plus、Pinia、Vue Router 和 Axios，包含管理员登录、会话、基础导航、用户运营操作、订单管理、内容缓存刷新、系统配置、运行诊断、系统日志和审计日志视图。
...
- `backend/system`：后端通用基础设施，包含统一响应、请求 ID、错误处理、用户级并发协调、后台系统配置、后台运行诊断、受限系统日志查看，以及可配置内存/Redis 存储的后端限流。
