...
[2026-07-17] backend/admin-web/withdrawal - 后台提现管理对接持久化 payout attempt：预览接口移除私钥和派生余额查询并要求提现读写双权限，单笔/批量最多处理 10 笔并按网络逐次提交严格 hex 私钥与 6 位 TOTP，仅 BROADCASTED、CONFIRMED 计为已提交，PREPARED 单独显示待广播；审计失败只记录 WARN 且不改变打款结果，前端使用 120 秒超时并将超时按后台可能仍在处理提示，覆盖所有退出路径的凭据清理。
