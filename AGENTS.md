...
- `admin-web`：后台管理网站，使用 Vue、Element Plus、Pinia、Vue Router 和 Axios，包含管理员登录、会话、基础导航、用户运营操作、订单管理、内容缓存刷新、系统配置、运行诊断和审计日志视图。
...
- `backend/system`：后端通用基础设施，包含统一响应、请求 ID、错误处理、用户级并发协调、后台系统配置、后台运行诊断，以及可配置内存/Redis 存储的后端限流。
