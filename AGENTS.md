...
- `backend/order`：充值与 VIP 订单模块；VIP TRC20 订单保存收款地址、USDT 合约、基础/应付金额和有效期不可变快照，每个用户仅保留一个活动订单，并通过数据库分配锁、活动金额唯一约束和交易哈希唯一约束防止错配与重放；自动与人工确认按 TronGrid fingerprint 分页覆盖订单时间窗，并对候选交易查询 receipt 和当前块，复核地址、合约、金额、付款时间、执行状态与真实确认数；原有效期限制付款时间，可配置确认宽限只延后最终过期；人工确认要求 `ORDER_WRITE` 与管理员 TOTP，自动确认和过期写系统审计，权益交付锁定用户行后续期；旧充值创建当前保持禁用。
