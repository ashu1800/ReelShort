...
- `backend/withdrawal`：积分提现申请和后台人工审批模块；默认 50 积分兑换 1 CNY，再按后台维护的 CNY/USD 汇率折算 TRC20 USDT，最低提现默认 10 USD，每笔申请保存人民币比例、汇率和最低金额快照；提交提现时冻结可用积分，后台确认 TRC 转账并录入 tx hash 后正式扣减积分，拒绝时释放冻结积分；审批和拒绝使用数据库锁保护提现单和积分账户，重复处理返回业务错误。
