...
- `backend/points`：积分账户、按视频时长计算的完成奖励、后台积分调整、充值入账和用户间积分交易模块；默认每完整 60 秒奖励 1 积分、短视频最低 1 积分，同一用户同一集永久只奖励一次；观看奖励和运营模拟观看共享每日全局规则快照与账号随机向下浮动额度，基础上限默认 1000、全局浮动上限默认 35%，按服务器自然日重置且只限制 `WATCH_REWARD` 自动奖励；积分交易按完整手机号账号归一化接收方，不扣手续费，并基于可用积分执行同事务扣减和入账；积分余额增加和入账路径带整型溢出保护，接收方溢出时转账整体回滚。
