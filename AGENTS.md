...
- `admin-web`：后台管理网站，使用 Vue、Element Plus、Pinia、Vue Router 和 Axios，包含管理员登录、会话、基础导航、用户运营操作、BLACKLISTED 状态管理、提现申请拒绝与最多 10 笔的单笔/批量自动打款、按网络展示原生币预计手续费、确认后展示实际手续费、打款 attempt 状态和链上确认进度、用户积分交易记录、带失败重试幂等键的积分调整、订单管理、内容缓存单语言刷新、双语货架刷新、货架健康状态和刷新任务状态、系统配置、运行诊断、内容源结构化诊断事件、异常告警、系统日志和审计日志视图；管理员登录支持可选 6 位 TOTP，已启用 2FA 的账号由后端强制验证；提现预览要求读写双权限且只提交提现 ID，私钥仅在最终执行请求中随 6 位 TOTP 提交，并在请求结束、弹层关闭和页面离开时清除；打款请求允许最多 120 秒，超时按状态不确定处理并刷新列表。
...
- `backend/withdrawal`：积分提现申请、后台逐次提交私钥的内存签名、持久化签名交易广播与链上确认模块；默认 50 积分兑换 1 CNY，再按后台维护的 CNY/USD 汇率折算 USDT，最低提现默认 10 USD，每笔申请保存汇率快照；TRON/ERC20/BEP20 预期热钱包公开地址为可选白名单，未配置时使用私钥派生地址，配置后必须匹配，签名阶段始终复核派生地址与持久化 intent；预览按网络聚合原生币预计手续费，TRON 使用 feeLimit 上限，EVM 使用当前 gasPrice 与 gasLimit；确认后将 TRON transaction info 顶层 fee 或 EVM receipt 的 gasUsed × effectiveGasPrice 作为实际手续费幂等写入 payout attempt，EVM receipt 缺少价格时只回退到 attempt 已持久化 gas price；TRON transfer 请求只编码 Base58Check payload 中去掉网络前缀后的 20 字节地址，节点返回的 `raw_data_hex` 通过 Trident protobuf 按本地打款 intent 严格校验后才签名，首次意图字段不匹配时只重建一次，第二次返回字段级安全原因码，且日志不记录私钥、签名或完整原始交易；EVM nonce 分配与 `SIGNING` intent 同事务，所有打款状态 mutation 固定按 withdrawal -> attempt 加锁；TRON 过期且单节点未找到时进入人工核对，禁止自动重签；提交提现时冻结可用积分，达到链上确认数后正式扣减积分，拒绝时释放冻结积分。
