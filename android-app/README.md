...
- 播放页：通过 `AppStateController` 获取播放地址并维护纯 Kotlin `PlaybackState`，HLS 平台播放器后续接入 Android 原生媒体播放方案。
