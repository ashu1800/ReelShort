...
适合当前 Windows 开发环境和单机部署形态。脚本通过 `docker compose exec postgres pg_dump` 导出 PostgreSQL，复制 `.env`、Compose 和 Nginx/部署文档等配置快照，并按保留天数清理旧备份。恢复脚本通过 `pg_restore --clean --if-exists` 恢复数据库，并明确需要先停止后端避免写入。
...
- `infra/.env`：部署密钥、数据库口令、站点配置、管理员配置。
