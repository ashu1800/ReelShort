...
引入 `ReelShortClient` 封装第三方 ReelShort 站点数据调用，Flask 路由只负责参数校验、调用 client、返回 Spring Boot 期望的 JSON 结构。

默认上游地址通过环境变量配置：

- `REELSHORT_SITE_URL`，默认 `https://www.reelshort.com`
- `REELSHORT_SITE_ID`，默认 `37`
- `REELSHORT_NEXT_BUILD_ID`，可选；未配置时从首页 HTML 自动发现
- `REELSHORT_REQUEST_TIMEOUT_SECONDS`，默认 `10`

如果 ReelShort 站点数据路径变化，优先通过环境变量或内容源服务内部实现调整，不改 Spring Boot 和 App。
