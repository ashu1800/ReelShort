...
补齐后台管理的持久化账号、角色和权限基础，让 Admin API 从“配置文件单管理员 + 已登录即可访问”升级为“数据库管理员账号 + 权限校验”。本阶段实施当时仍保持 `admin / Admin123` 在本地开发和测试中可登录，避免破坏已有测试；当前该固定账号仅允许显式 `app-dev` 和测试 profile 使用，生产必须提供独立 BCrypt 密码哈希，缺失或使用已知开发值时拒绝启动。
...
- 本阶段实施当时默认 `admin / Admin123` 登录继续用于本地开发和测试；当前仅显式 `app-dev` 和测试 profile 保留该兼容行为，生产不提供默认密码回退。
- 现有后台接口路径和响应结构不变。
