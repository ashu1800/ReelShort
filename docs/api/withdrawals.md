...
- TRON RPC 默认按 `250ms` 间隔启动请求；可安全重放的查询和未签名交易构建遇到 HTTP 429 时最多按 `Retry-After` 或 1/2/4 秒退避重试，链上 Energy/带宽价格成对缓存 5 分钟。重试耗尽会明确返回“节点请求过于频繁、打款未执行”。签名交易广播始终只发送一次，不进入自动重试；生产建议配置可选的 `REELSHORT_TRON_API_KEY` 提升 TronGrid 配额。
